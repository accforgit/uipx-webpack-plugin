...
class UIPXWebpackPlugin {
  ...
  apply(compiler) {
    if (typeof this.options.enable === "boolean" && !this.options.enable)
      return;
    // 将插件入口文件加入到 entry 中
    let indexPath = path.resolve(__dirname, "./src/index.js");
    compiler.plugin("entry-option", (locale, entry) => {
      const type = Object.prototype.toString.call(entry).slice(8, -1);
      if (type === "String") {
        entry = [indexPath, entry];
        console.warn(
          "[uipx-webpack-plugin] 暂不支持 entry 为 string 类型的情况\n"
        );
      } else if (type === "Array") {
    ...
    compiler.plugin("emit", (compilation, callback) => {
      try {
        const data = `import initRegisterListener from './registerListener';initRegisterListener(${JSON.stringify(
          this.options
        )})`;
