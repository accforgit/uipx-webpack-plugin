...
export default function handler({ name, data }, callback) {
	...
	const rqMap = {
		...
		[VARS.pickColorName]() {
			//canvas 添加点击监听事件
			canvasClick.change(data, callback);
		},
		[VARS.clickThroughName]() {
			canvas.style.pointerEvents = data ? 'none' : 'auto';
		},
	};
	const fn = rqMap[name];
...
const canvasPosition = {
	...
	y: 0,
	moveBy(x, y) {
		canvas.style.transform = `translate(${x === null ? this.x : this.x + x}px, ${
			y === null ? this.y : this.y + y
		}px)`;
	},
	reset() {
		this.x = 0;
...
const canvasTouch = {
	...
	touchmoveFn(e) {
		e.preventDefault();
		canvasPosition.moveBy(
			e.changedTouches[0].clientX - canvasTouch.startX,
			e.changedTouches[0].clientY - canvasTouch.startY
		);
	},
	touchendFn(e) {
		e.preventDefault();
...
const canvasClick = {
	change(isPickColor, fn = () => {}) {
		if (isPickColor) {
			canvas.addEventListener('touchstart', this.click.bind(this, fn), { passive: false });
		} else {
			canvas.removeEventListener('click', this.click.bind(this, fn), { passive: false });
		}
	},
	click(fn, event) {
		var x = event.changedTouches[0].clientX - canvasPosition.x;
		var y = event.changedTouches[0].clientY - canvasPosition.y;
		let ctx = invokeImg.getCtx();
